...
### 1주차 빌드 스코프 — MVP에서 더 축소 (보스 제외, 1웨이브만)
- **결정**: 2026-05-19 MVP 결정을 더 축소. 1주차 최종 빌드 목표:
  - **1웨이브 몬스터 스폰**
  - **구현된 공격(좌클릭 콤보 + 강공 + 회피)으로 몬스터 처치**
  - **레벨업 시스템 (자동 분배)**
- **제외**: 보스(#B1). 시간 남으면 보스 또는 다른 컨텐츠로 추가.
- **이유**:
  - #5~#7까지 진행하며 인프라(StateMachine, IsInvincible, Cooldown 패턴, Animation Event 등)에 시간 투자가 컸음 — 학습 가치는 충분하지만 일정 압박.
  - 1웨이브 + 처치 + 레벨업으로도 *게임 루프 완결성*은 확보 가능 — 시작 → 적 등장 → 처치 → 성장 → 끝.
  - 보스는 2주차에 정식 구현이 더 의미 있음 (스킬·이펙트 등 함께).
- **영향**:
  - `#B1 보스` — 1주차에서 빼고 2주차 또는 시간 남을 때 후순위.
  - `#S2 웨이브` — 1웨이브만, 코드 매우 단순화.
  - `#M1 몬스터` — 1종(기본 근접)만 필수. 강화/원거리는 2주차.
  - 1주차 빌드 = "최소 게임 루프" — 시작 → 1웨이브 스폰 → 적 처치 → 레벨업 → 클리어 텍스트.
- **시간 남을 시 우선순위**: 보스(#B1) > 강화 몬스터 > 원거리 몬스터 > 폴리싱(타격감, 이펙트).
